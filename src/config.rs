...
pub fn default_config() -> Result<Config> {
    ...
    let data_root = PathBuf::from(".mcp-search");
    Ok(Config {
        roots: Vec::new(),
        ...
        language_pack_root: data_root.join("languages"),
        languages: None,
        ...
        embedder: EmbedderConfig {
            ...
            fastembed: Some(FastembedConfig {
                ...
                model_path: ".mcp-search/models/bge".to_string(),
            }),
            ollama: None,
            openai: None,
        },
        lancedb_path: data_root.join("lancedb"),
        log_path: None,
    })
}
    ...
    json!({
        ...
        "properties": {
            ...
            "data_root": {
                "type": "string",
                "default": ".mcp-search",
                "description": "Base directory for language packs and LanceDB data."
            },
            "language_pack_root": {
                "type": "string",
                "default": ".mcp-search/languages",
...
pub fn apply_config_override(base: Config, value: &Value, cwd: &PathBuf) -> Result<Config> {
    ...
    if let Some(value) = obj.get("data_root") {
        let text = value
            .as_str()
            .ok_or_else(|| anyhow!("data_root must be a string"))?;
        let root = if text.trim().is_empty() {
            PathBuf::from(".mcp-search")
        } else {
            PathBuf::from(text)
        };
        next.data_root = root.clone();
        next.language_pack_root = root.join("languages");
        next.lancedb_path = root.join("lancedb");
    }
        ...
        match key.as_str() {
            ...
            "roots" => {
                ...
                if inputs.is_empty() {
                    let default_root = root_from_cwd(cwd);
                    next.roots = Vec::new();
                    next.default_root = default_root.path;
                    next.default_root_canon = default_root.path_canon;
                } else {
                    let roots = build_root_configs(&inputs, cwd)?;
            ...
            "language_pack_root" => {
                let text = value
                    .as_str()
                    .ok_or_else(|| anyhow!("language_pack_root must be a string"))?;
            ...
            "language_download" => {
                if !value.is_null() {
                    next.language_download = value
                        .as_bool()
                        .ok_or_else(|| anyhow!("language_download must be a boolean"))?;
                }
            }
            "log_path" => {
                if !value.is_null() {
                    let text = value
                        .as_str()
                        .ok_or_else(|| anyhow!("log_path must be a string"))?;
            ...
            "lancedb" => {
                let obj = value
                    .as_object()
                    .ok_or_else(|| anyhow!("lancedb must be an object"))?;
                if let Some(path_value) = obj.get("path") {
                    let text = path_value
                        .as_str()
                        .ok_or_else(|| anyhow!("lancedb.path must be a string"))?;
...
fn parse_root_inputs(value: &Value) -> Result<Vec<RootInput>> {
    let inputs: Vec<RootInput> =
        serde_json::from_value(value.clone()).map_err(|err| anyhow!("invalid roots: {}", err))?;
    Ok(inputs)
}
    ...
    match value {
        Value::Array(items) => {
            ...
            for item in items {
                let text = item
                    .as_str()
                    .ok_or_else(|| anyhow!("{} must be strings", name))?;
                if !text.trim().is_empty() {
...
fn parse_embedder_config(value: &Value) -> Result<EmbedderConfig> {
    let obj = value
        .as_object()
        .ok_or_else(|| anyhow!("embedder must be an object"))?;
    let provider = obj
        .get("provider")
        .and_then(Value::as_str)
        .ok_or_else(|| anyhow!("embedder.provider is required"))?
        .to_string();
    let concurrency = obj
        .get("concurrency")
    ...
    match provider.as_str() {
        ...
        "openai" => {
            ...
            if cfg.api_key_env.trim().is_empty() || cfg.model.trim().is_empty() {
                return Err(anyhow!(
...
fn parse_usize_value(value: &Value, name: &str) -> Result<usize> {
    let num = value
        .as_u64()
        .ok_or_else(|| anyhow!("{} must be an integer", name))?;

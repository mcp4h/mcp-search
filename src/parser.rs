...
pub struct LanguageRegistry {
    ...
    by_name: HashMap<String, Arc<LoadedLanguage>>,
}

pub fn load_language(pack: &LanguagePack) -> Result<LoadedLanguage> {
    let lib = unsafe { Library::new(&pack.parser_path) }
        .map_err(|err| anyhow!("failed to load {}: {}", pack.parser_path.display(), err))?;
    ...
    let language = unsafe {
        let func: libloading::Symbol<unsafe extern "C" fn() -> Language> = lib
            ...
            .map_err(|err| anyhow!("missing symbol {}: {}", symbol_name, err))?;
        func()
    };
    let query_source = std::fs::read_to_string(&pack.structural_query_path).map_err(|err| {
        anyhow!(
            "failed to read {}: {}",
    ...
    Ok(LoadedLanguage {
        ...
        extensions: pack.config.extensions.clone(),
        language,
        structural_query,
        _lib: Arc::new(lib),
    })
...
pub fn build_registry(packs: &[LanguagePack]) -> Result<LanguageRegistry> {
    ...
    for pack in packs {
        ...
        by_name.insert(loaded.name.to_lowercase(), Arc::clone(&loaded));
...
pub fn detect_language(registry: &LanguageRegistry, path: &Path) -> Option<Arc<LoadedLanguage>> {
    ...
    registry.by_extension.get(&ext.to_lowercase()).cloned()
}

pub fn detect_language_by_name(
    registry: &LanguageRegistry,
    name: &str,
) -> Option<Arc<LoadedLanguage>> {
    registry.by_name.get(&name.to_lowercase()).cloned()
}
